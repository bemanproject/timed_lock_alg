...
- Added feature test macro `__cpp_lib_multi_lock`.
- Added motivation example with `Account` class as a *Cpp17TimedLockable*.
- Added design discussion for the `mutex()` accessor.
...
5. [Design Decisions](#design-decisions)
   ...
   - [The `mutex()` accessor](#the-mutex-accessor)
...
**Example 5: Account transfer with a deadline**
...
public:
    ...
    bool try_lock_for(const std::chrono::duration<Rep, Period>& d) {
        return mtx_.try_lock_for(d);
    }
    template<class Clock, class Duration>
    bool try_lock_until(const std::chrono::time_point<Clock, Duration>& tp) {
        return mtx_.try_lock_until(tp);
    }

    // Only call while locked:
    int  balance() const { return balance_; }
    void deposit(int n)  { balance_ += n; }
    void withdraw(int n) { balance_ -= n; }
};

template<class Clock, class Duration>
bool transfer(Account& from, Account& to, int amount,
              const std::chrono::time_point<Clock, Duration>& deadline) {
    std::multi_lock lock(deadline, from, to);
    if (!lock) return false;  // could not acquire both accounts in time

    from.withdraw(amount);
    to.deposit(amount);
    return true;
    // both accounts unlocked automatically
}
```

Without `std::multi_lock`, the equivalent code requires either manually managing lock state on every path, or combining `std::try_lock_until` from P3832 with `std::scoped_lock(std::adopt_lock, ...)` — both more verbose and error-prone than the single-line RAII construction above.
...
`std::multi_lock` provides a `mutex()` member function returning `mutex_type` (a `tuple<MutexTypes*...>`). This is motivated by `std::multi_lock` being designed to be able to work as a generic replacement for `std::unique_lock` that also handles multiple mutexes. Since `std::unique_lock` exposes a `mutex()` accessor, `std::multi_lock` must provide one as well to serve as a drop-in replacement in generic code. [N2406](https://wg21.link/n2406) mentioned that some algorithms may need access to the referenced mutex, and `multi_lock` stays true to that sentiment.
...
```cpp
#define __cpp_lib_multi_lock  20XXXXL // also in <mutex>
```
...
- [N2406](https://wg21.link/n2406): Mutex Types and Locking
- N5046: Working Draft, Standard for Programming Language C++
